...
public record CraftThiefProtectionOptions(
        ...
        @SerializedName("message") String[] message) implements ThiefProtectionOptions {
    ...
    @Contract("null -> new")
    public static @NotNull CraftThiefProtectionOptions load(@Nullable ConfigurationSection section) {
        if (section == null)
            return new CraftThiefProtectionOptions(false, DEFAULT_PERMISSION, DEFAULT_BYPASS_PERMISSION, DEFAULT_DURATION, null, 1F, 1F, null);
        ...
        return new CraftThiefProtectionOptions(enabled, permission, bypassPermission, expiration, sound, volume, pitch, messageArray);
    }
